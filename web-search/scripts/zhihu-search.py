...
# 20s（原 5s）：知乎 OpenAPI 在国内网络（联通 + GFW）下 5s 极易超时；而知乎站内
# 是中文查询的默认引擎（SKILL.md「铁律：默认并行带知乎」），一旦超时整条默认路径就没结果。
...
def emit_formatted(body_text: str) -> None:
    """Default: one line per item: "Title | Url | AuthorName | VoteUpCount | ContentText[:200]"."""
    secret = os.getenv("ZHIHU_ACCESS_SECRET", "")
    if secret:
        body_text = body_text.replace(secret, "***")
    try:
    ...
    for i in items:
        ...
        author = str(i.get("AuthorName") or "")
        vote = i.get("VoteUpCount", "")
        content = str(i.get("ContentText") or "")[:200]
        print(f"{title} | {url} | {author} | {vote} | {content}")
...
def request_zhihu(query: str, count: int) -> str:
    ...
    for attempt in range(RETRY_COUNT + 1):
        ...
        die(f"Zhihu request failed after {RETRY_COUNT + 1} attempt(s): {last_exc}")
...
def main() -> None:
    ...
    body_text = request_zhihu(query, count)
    if raw_flag:
        emit_raw(body_text)
    else:
